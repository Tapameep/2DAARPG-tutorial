...
func add_item( item : ItemData, count : int = 1 ) -> bool:
	for s in slots: # for items already in the inventory
		if s:
			if s.item_data == item:
				s.quantity += count
				return true # successful pickup of item
				
	for i in inventory_slots().size():  # if item not already in inventory
		if slots[ i ] == null:
...
func item_from_save( save_object : Dictionary ) -> SlotData: # take dicitonary and convert to slot data
	if save_object.item == "":
		return null
...
func equip_item( slot : SlotData ) -> void:
	...
	PauseMenu.focused_item_change( unequiped_slot )
	
	pass


func get_attack_bonus() -> int:
	return get_equipment_bonus( EquipableItemModifer.Type.ATTACK )


func get_attack_bonus_diff( item : EquipableItemData ) -> int:
	var before : int = get_attack_bonus()
	var after : int = get_equipment_bonus( EquipableItemModifer.Type.ATTACK, item )
	
	return after - before


func get_defense_bonus() -> int:
	return get_equipment_bonus( EquipableItemModifer.Type.DEFENSE )


func get_defense_bonus_diff( item : EquipableItemData ) -> int:
	var before : int = get_defense_bonus()
	var after : int = get_equipment_bonus( EquipableItemModifer.Type.DEFENSE, item )
	
	return after - before


func get_equipment_bonus( bonus_type : EquipableItemModifer.Type, compare : EquipableItemData = null ) -> int:
	var bonus : int = 0
